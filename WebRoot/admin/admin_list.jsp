            ...
            <form  method="post" id="mainform">
                ...
                <div id="data">            
                    <table id="datalist">
                        ...
                        <s:iterator value="dataList" var="l">
                        <tr>
                            <td><input type="checkbox" name="adminId" value="<s:property value="AdminInfo.id"/>"/></td>
                            <td><s:property value="AdminInfo.id"/> </td>
                            <td><s:property value="AdminInfo.name"/></td>
                            <td><s:property value="AdminInfo.admin_code"/> </td>
                            <td><s:property value="AdminInfo.telphone"/> </td>
                            <td><s:property value="AdminInfo.email"/> </td>
                            <td><s:property value="AdminInfo.enrolldate"/></td>
                            <td>
                                <a class="summary"  onmouseover="showDetail(true,this);" onmouseout="showDetail(false,this);">管理员管理</a>
                                <!--浮动的详细信息-->
                                <div class="detail_info">
                                <s:iterator value="AdminInfo.roleList" var="r">
                                    <s:property value="#r.role_name"/>
                                </s:iterator>
                                </div>
                            </td>
                            <td class="td_modi">
                                <input type="button" value="修改" class="btn_modify" onclick="location.href='../control/adminUpdateControl?m.id=7&operation=u&adminId=${l.adminInfo.id}';" />
                                <input type="button" value="删除" class="btn_delete" onclick="deleteAdmin(${l.adminInfo.id});" />
                            </td>
                        </tr>
                          </s:iterator>
                    </table>
                ...
                <!--分页-->
                <div id="pages">
        	        <s:if test="pageNum>1"><a href="AdminInfoList?pageNum=${pageNum-1}">上一页</a></s:if>
                <s:else>上一页</s:else>
        	        <s:iterator value="new int[totalPages]" status="i">
        	        <s:if test="#i.count==pageNum">
        	           <a href="AdminInfoList?pageNum=<s:property value='#i.count'/>" class="current_page"><s:property value="#i.count"/></a>
        	        </s:if>
        	        <s:else>
        	          <a href="AdminInfoList?pageNum=<s:property value='#i.count'/>"><s:property value="#i.count"/> </a>
        	        </s:else>
        	        </s:iterator>
        	        <s:if test="pageNum<totalPages">
        	        <a href="AdminInfoList?pageNum=${pageNum+1}">下一页</a>
        	        </s:if>
        	        <s:else>下一页</s:else>
                </div>  
            </form>
